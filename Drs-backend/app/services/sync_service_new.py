    ...
    async def apply_snapshot(
        ...
        try:
            ...
            if not existing_entity:
                ...
                # ================================================================
                # SHORE-SIDE UNIVERSAL SMART MERGE (Defect only)
                #
                # The Shore receives DELTA payloads from the Vessel (only changed cols).
                # Rules:
                #   ABSOLUTE: Vessel's PENDING_CLOSURE always accepted (offline protection)
                #   ABSOLUTE: Permanent defect_number never overwritten by temp T- number
                #   NON-STATUS COLUMNS: Last Write Wins by updated_at timestamp
                # ================================================================
                if model_class == Defect and existing_entity:
                    STATUS_COLUMNS = {
                        'status', 'closure_remarks', 'closure_requested_at',
                        'closed_at', 'closed_by_id'
                    }
                    SHORE_PROTECTED_COLUMNS = {'defect_number'}

                    incoming_status = data.get('status')
                    raw_status = getattr(existing_entity, 'status', None)
                    current_status = raw_status.value if hasattr(raw_status, 'value') else str(raw_status or '')
                    ...
                    # BACKWARD COMPAT: Old Vessel delta payloads (exclude_unset=True) do NOT
                    # include updated_at. In that case we cannot compare timestamps, so we
                    # treat incoming as valid and accept it — same as the old version-based logic.
                    can_compare = incoming_updated_at is not None and local_updated_at is not None
                    if can_compare:
                        incoming_is_newer = incoming_updated_at > local_updated_at
                    else:
                        # No timestamp to compare — default to accepting incoming (old behaviour)
                        incoming_is_newer = True

                    # ---- STEP 1: STATUS resolution ----
                    # Shore receives from Vessel — only Vessel can send PENDING_CLOSURE.
                    # Shore's own CLOSED/OPEN decisions are already in the DB here.
                    # So the only tricky case is: Vessel sent PENDING_CLOSURE while Shore
                    # changed something (making Shore's timestamp newer). We ALWAYS accept
                    # ---- SHORE STATUS RULES (applied in priority order) ----
                    #
                    # RULE 0 (ABSOLUTE): If Shore has CLOSED → no Vessel push can ever
                    #   change it back. Only a Shore admin can reopen via the main API.
                    #
                    # RULE 1 (ABSOLUTE): If Vessel sends PENDING_CLOSURE and Shore has
                    #   OPEN → always accept, even if Shore's timestamp is newer.
                    #   (This is the offline closure protection.)
                    #
                    # RULE 2: Anything else → Last Write Wins by timestamp.
                    #   If no timestamp available (old Vessel format) → accept incoming.
                    apply_incoming_status = False

                    if current_status == 'CLOSED':
                        # RULE 0: Shore CLOSED is terminal — Vessel cannot change it
                        apply_incoming_status = False
                        logger.info(f"🔒 SHORE PROTECTING CLOSED status — Vessel push '{incoming_status}' ignored for {entity_id}")

                    elif incoming_status == 'PENDING_CLOSURE' and current_status == 'OPEN':
                        # RULE 1: Vessel closure request — always accept on Shore
                        apply_incoming_status = True
                        logger.info(f"✅ SHORE ACCEPTED PENDING_CLOSURE for {entity_id}")

                    else:
                        # RULE 2: Last Write Wins (or accept if no timestamp)
                        apply_incoming_status = incoming_is_newer or not local_updated_at


                    if apply_incoming_status:
                        for col in STATUS_COLUMNS:
                            if col in clean_data and hasattr(existing_entity, col):
                                setattr(existing_entity, col, clean_data[col])
                    else:
                        logger.info(f"🛡️ SHORE PROTECTED STATUS '{current_status}' over incoming '{incoming_status}'")

                    # ---- STEP 2: Protect permanent defect_number ----
                    existing_dn = getattr(existing_entity, 'defect_number', None)
                    incoming_dn = clean_data.get('defect_number', '')
                    is_incoming_temp = incoming_dn and str(incoming_dn).startswith('T-')
                    is_existing_permanent = existing_dn and not str(existing_dn).startswith('T-')
                    if is_existing_permanent and is_incoming_temp:
                        clean_data.pop('defect_number', None)
                        logger.info(f"🔢 PROTECTED permanent defect_number '{existing_dn}'")
                    elif not is_incoming_temp and incoming_dn and not is_existing_permanent:
                        setattr(existing_entity, 'defect_number', incoming_dn)

                    # ---- STEP 3: Non-status columns — Last Write Wins ----
                    for key, value in clean_data.items():
                        if key in STATUS_COLUMNS or key in SHORE_PROTECTED_COLUMNS:
                            continue
                        if not hasattr(existing_entity, key):
                            continue
                        col_def = model_class.__table__.columns.get(key)
                        if col_def is not None and not col_def.nullable and value is None:
                            continue
                        if incoming_is_newer or not local_updated_at:
                            setattr(existing_entity, key, value)
                        else:
                            logger.debug(f"⏱️ SHORE SKIP '{key}': local is newer for {entity_id}")

                    if hasattr(existing_entity, 'version'):
                        existing_entity.version = max(current_version, incoming_version)

                else:
                    # ---- GENERIC (non-Defect) UPDATE — version + origin based ----
                    if incoming_version > current_version:
                        ...
                        if incoming_origin == 'SHORE' and local_origin != 'SHORE':
                            logger.info(f"⬇️ SHORE WINS: {model_class.__tablename__} {entity_id}")
                            for key, value in clean_data.items():
                                if hasattr(existing_entity, key):
                                    setattr(existing_entity, key, value)
                            ...
                            for key, value in clean_data.items():
                                ...
                                if str(getattr(existing_entity, key)) != str(value):
                                    ...
                                    break
                            if not has_diff:
                                logger.info(f"⏭️ SYNC SKIP: {model_class.__tablename__} {entity_id} — no changes")
                                return
                            else:
                                logger.info(f"🔁 SYNC RECONCILE: {model_class.__tablename__} {entity_id}")
                                for key, value in clean_data.items():
                                    if hasattr(existing_entity, key):
                                        setattr(existing_entity, key, value)
                        else:
                            ...
                            if incoming_updated_at and local_updated_at:
                                ...
                                if incoming_updated_at > local_updated_at:
                                    logger.info(f"⏱️ LAST WRITE WINS: {model_class.__tablename__} {entity_id}")
                                    for key, value in clean_data.items():
                                        if hasattr(existing_entity, key):
                                            setattr(existing_entity, key, value)
                                else:
