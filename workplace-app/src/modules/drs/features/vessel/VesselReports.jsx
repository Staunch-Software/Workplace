...
const VesselReports = () => {
    ...
    if (filters.text_sort?.field) {
      ...
      if (key) {
        result.sort((a, b) => {
          ...
          // 3. Logical Status Sort (Workflow order)
          if (field === 'status') {
          ...
          // 4. Alphabetical (Equipment, Source, Description)
  ...
  const handleExport = () => {
    ...
    // Add vessel IMO filter for vessel users
    if (vesselImo) {
      activeFilters['vessel_imo'] = [vesselImo];
    }
  ...
  const handleDownloadTemplate = async () => {
    try {
      await defectApi.downloadVesselTemplate();
    } catch (err) {
  ...
  const previewData = filteredData.slice(0, 50); // Show first 50 rows in preview
        ...
        <table className="data-table" style={{ tableLayout: 'fixed', minWidth: '1400px' }}>
          ...
          <tbody>
            {previewData.length === 0 ? (
              <tr>
                <td colSpan={visibleColumns.length + 1} style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "24px" }}>
              ...
              {/* STEP 2 */}
              <h3 style={{ marginBottom: "10px", fontSize: "14px", color: "#1e293b" }}>
