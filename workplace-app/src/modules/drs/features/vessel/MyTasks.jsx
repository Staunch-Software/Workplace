...
// Priority colors matching ShoreDashboard exactly
...
// ── Description tooltip (wider, wraps) ────────────────────────────────────
const DescTooltip = ({ text, children }) => {
  ...
  const handleMouseEnter = () => {
    if (ref.current) {
      ...
      const below = spaceAbove < 180; // not enough room above
      setPos({
  ...
  return (
    <span
      ref={ref}
      className="desc-tooltip-trigger"
...
const LiveFeed = () => {
  ...
  // 1. Fetch ALL — no params, no refetch on filter change
  const { data: feedItems = [], isLoading, refetch, isFetching } = useQuery({
    queryKey: ['live-feed'],           // ← static key, never changes
    queryFn: () => defectApi.getLiveFeed(),
    refetchInterval: 15000,
  });

  // ── Build vessel options from actual feed data ──
  const uniqueVessels = useMemo(() => {
    const seen = new Map();
    feedItems.forEach(item => {
      if (item.vessel_imo && item.vessel_name) {
        seen.set(String(item.vessel_imo), item.vessel_name);
      }
    });
    return [...seen.entries()].map(([imo, name]) => ({ value: imo, label: name }));
  }, [feedItems]);

  // Replace vesselOptions with:
  const vesselOptions = uniqueVessels;
  // Mark read mutation
  ...
  const handleView = (defectId) => {
    window.open(`/drs/vessel/dashboard?highlightDefectId=${defectId}`, '_blank');
  };
  ...
  // Replace your sortedFeedItems with this:
    ...
    <div className="live-feed-page">
      ...
      <div className="feed-header">
        <div className="header-top-row">
          ...
          {/* Replace the Refresh button row with this */}
            ...
            <div className="view-mode-switcher">
              {[
                ...
                <button
                  ...
                  className={`view-mode-btn ${viewMode === opt.key ? 'is-active' : ''}`}
                >
                  {opt.label}
