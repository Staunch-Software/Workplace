...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || (isDev ? 'http://localhost:8005' : '/aepms/api');
...
class ApiService {
  ...
  async request(endpoint, options = {}) {
    ...
    if (
      token &&
      !endpoint.includes("/auth/local/login") &&
      !endpoint.includes("/auth/sso/microsoft")
    ) {
    ...
    try {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          if (response.status === 422 && Array.isArray(errorData.detail)) {
            ...
            errorMessage = `Validation Error (422):\n${errorData.detail
              .map((d) => `[Location: ${d.loc.join(".")}] ${d.msg}`)
              .join("; ")}`;
          } else {
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        throw new Error(
          `Cannot connect to server at ${this.baseURL}. Please ensure backend is running.`,
        );
      }
  ...
  async getFleetConfigurationSummary() {
    ...
    try {
      ...
      console.error(
        "Backend endpoint failed. Dashboard will display zeros.",
        error,
      );
      return {
        total_ships: 0,
  ...
  async getFleetShopTrialSummary() {
    ...
    try {
      ...
      console.warn(
        "Using mock data for fleet shop trial summary in Fleet.jsx fallback:",
        error,
      );
      return {
        summary: [
  ...
  async getMEHistoryDetails(imoNumber) {
    console.log(
      "📜 Get ME History Details (Actuals + Devs) for IMO:",
      imoNumber,
    );
  ...
  async getPerformanceHistory(imoNumber, limit, referenceMonth = null) {
    console.log("📊 Get Performance History:", {
      imoNumber,
      limit,
      referenceMonth,
    });
    ...
    if (referenceMonth) {
      ...
      if (cleanedRefMonth) {
        ...
        console.warn(
          `Warning: referenceMonth was invalid ('${referenceMonth}'). Ignoring ref_month.`,
        );
      }
    }
  ...
  async getRawReportUrl(reportId, engineType) {
    console.log("📥 Fetching Raw Report Download URL:", {
      reportId,
      engineType,
    });
    // This matches the @app.get("/api/performance/raw-download-link/{report_id}") backend endpoint
    return this.request(
      `/api/performance/raw-download-link/${reportId}?engine_type=${engineType}`,
    );
  }

  // Replace these specific functions inside your ApiService class:

  // ==================== DIRECT AI EXTRACTION ENDPOINTS ====================

  /**
   * Fetches vessels assigned to the user from the Control Database
   */
  async getUserVessels() {
    return this.request("/api/v1/user/vessels");
  }

  /**
   * Fetches auxiliary generator units for a specific ship
   */
  async getGenerators(imoNumber) {
    return this.request(`/aux/generators/${imoNumber}`);
  }

  /**
   * Sends a PDF to Gemini for extraction
   */
  async extractPdf(file, engineType) {
    console.log("🤖 AI Extraction request initiated");
    const formData = new FormData();
    formData.append("file", file);
    formData.append("engine_type", engineType);

    // Use this.request instead of this.client.post
    return this.request("/api/performance/extract-pdf", {
      method: "POST",
      body: formData,
    });
  }

  async saveAiBaseline(payload) {
    return this.request("/api/admin/save-ai-baseline", {
      method: "POST",
  ...
  /**
   * Saves the confirmed/edited data into the Performance Database
   */
  async saveConfirmedData(data, engineType, vesselId) {
    console.log("💾 Saving confirmed data to database for:", vesselId);
    return this.request("/api/performance/save-confirmed-json", {
      method: "POST",
      body: JSON.stringify({
        data: data,
  ...
  async getBatchRawZip(reportIds, engineType) {
    ...
    const response = await fetch(
      `${this.baseURL}/api/performance/batch-download-zip`,
      {
        method: "POST",
        headers: {
  ...
  async getMEDashboardSummary(year, month = null, imoNumber = null) {
    ...
    return this.request(
      `/api/performance/me-dashboard-summary?${params.toString()}`,
    );
  }
  ...
  // }
  async getAuxiliaryPerformanceHistory(
    imoNumber,
    limitInput,
    ref_month = null,
    generatorId,
  ...
  async uploadAuxReport(imoNumber, file) {
    ...
    const response = await fetch(
      `${this.baseURL}/aux/upload-auxiliary-report/`,
      {
        method: "POST",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: formData,
  ...
  async getAEDeviationHistoryTable(generatorId, refDate = null) {
    console.log(
      "📊 Get AE Deviation Table History for:",
      generatorId,
      "Ref Date:",
      refDate,
    );
  ...
  async getAEDashboardSummary(year, month = null, imoNumber = null) {
    ...
    return this.request(
      `/api/performance/ae-dashboard-summary?${params.toString()}`,
    );
  }
  ...
  async getMainEngineDeviationHistory(imoNumber, refDate = null) {
    console.log(
      "📊 Get ME Deviation Table History for:",
      imoNumber,
      "Ref Date:",
      refDate,
    );
  ...
  async uploadVesselManualReport(formData) {
    ...
    const response = await fetch(
      `${this.baseURL}/api/luboil/vessel/manual-upload`,
      {
        method: "POST",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: formData,
  ...
  async generateSasUrl(blobUrl) {
    ...
    try {
      const res = await this.request(
        `/api/blob/freshen-url?blob_url=${encodeURIComponent(blobUrl)}`,
      );
  ...
  async getVesselMentions(imoNumber, chatMode = "external") {
    console.log(
      "👥 Fetching Vessel Mentions for IMO:",
      imoNumber,
      "Mode:",
      chatMode,
    );
    return this.request(
      `/api/luboil/mentions/${imoNumber}?chat_mode=${chatMode}`,
    );
  }
  ...
  async getLuboilReports(imoNumber) {
    ...
    for (const report of sortedReports) {
      ...
      if (diffDays >= 25) {
        ...
        console.log(
          `Skipping superseded report from ${currentReportDate.toISOString().split("T")[0]}`,
        );
      }
  ...
  async getVoyageSummary(imoNumber, voyageNo) {
    console.log(
      `🚢 Fetching Voyage Summary: IMO ${imoNumber}, Voyage ${voyageNo}`,
    );
  ...
  async getVoyageAnalytics(imoNumber, voyageNo) {
    console.log(
      `📊 Fetching Voyage Analytics: IMO ${imoNumber}, Voyage ${voyageNo}`,
    );
  ...
  async getVoyageEmissions(imoNumber, year) {
    console.log(
      `🌿 Fetching Emission Details for IMO ${imoNumber}, Year ${year}`,
    );
  ...
  async deleteReport(reportId, engineType) {
    ...
    return this.request(
      `/api/performance/delete-report/${reportId}?engine_type=${engineType}`,
      {
        method: "DELETE",
      },
    );
