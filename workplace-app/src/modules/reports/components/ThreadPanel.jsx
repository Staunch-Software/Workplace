// src/modules/reports/components/ThreadPanel.jsx
import React, { useState, useEffect, useRef } from "react";
import { Send, MessageSquare, ChevronDown, ChevronUp, Paperclip, X, Loader2, Download } from "lucide-react";
...
const ThreadAttachmentLink = ({ reportId, threadId, attachment, isMine }) => {
  ...
  const handleDownload = async () => {
    try {
      setDownloading(true);
      const { url } = await reportsApi.getThreadAttachmentUrl(reportId, threadId, attachment.id);
...
export default function ThreadPanel({ reportId, reportName, reportMeta, isCollapsed, onToggle }) {
  const { user } = useAuth();
  ...
  const { data: threads = [], isLoading } = useQuery({
    queryKey: ["report-threads", reportId],
    queryFn: () => reportsApi.getThreads(reportId),
    enabled: !!reportId,
  ...
  const mutation = useMutation({
    mutationFn: ({ body, tagged_user_ids, attachments }) =>
      reportsApi.postThread(reportId, { body, tagged_user_ids, attachments }),
    onSuccess: () => {
      setDraft("");
      setTaggedUsers([]);
      setSelectedFile(null);
      setIsUploading(false);
      queryClient.invalidateQueries(["report-threads", reportId]);
  ...
  async function handleSend() {
    if ((!draft.trim() && !selectedFile) || mutation.isPending || isUploading) return;
    ...
    if (selectedFile) {
      try {
        ...
        await reportsApi.uploadToAzureBlob(selectedFile, sasData.url);
    ...
    mutation.mutate({ body: draft.trim(), tagged_user_ids: taggedUsers, attachments });
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ padding: '16px 24px', borderTop: '1px solid #e2e8f0', background: '#fff' }}>
        ...
        <div style={{ 
          ...
          <button
            onClick={handleSend}
            disabled={(!draft.trim() && !selectedFile) || mutation.isPending || isUploading}
            style={{
