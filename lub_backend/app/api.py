...
async def get_luboil_reports(
    ...
    try:
        ...
        if str(imo_number) not in [str(x) for x in allowed_imos]:
            logger.warning(f"Unauthorized access attempt to IMO {imo_number}")
            raise HTTPException(
                status_code=403, 
...
async def upload_luboil_report(
    ...
    try:
        ...
        try:
            # Get the actual safe filename generated by the DB (e.g., file_v2.pdf)
            actual_filename = result.get("file_name", file.filename)
            
            # Clean filename (Replace spaces with underscores to prevent URL issues)
            clean_filename = actual_filename.replace(" ", "_").replace("(", "").replace(")", "")
        ...
        # =========================================================
        # 🔥 IMPROVED LIVE FEED TRIGGER (Multi-line Structured Message)
        # =========================================================
        try:
            ...
            # FIX: Skip creating a feed event for duplicate uploads.
            # Previously every upload (including re-uploads of the same PDF) created a
            # LuboilEvent, causing the report to appear twice in the feed.
            if is_duplicate:
                logger.info(f"♻️  Duplicate report for {vessel_name} — skipping Live Feed entry.")
            else:
                # 1. Build the structured message only for genuinely new reports
                ...
                db.add(new_event)
                await db.commit()
                logger.info(f"✅ Live Feed updated for {vessel_name} (new report).")
    ...
    try:
        ...
        # CASE B: Shore User is making a decision (ACCEPT/DECLINE)
            ...
            if request.approval_action == 'ACCEPT':
                ...
                timestamp_str = datetime.utcnow().strftime("%d/%m/%Y %H:%M")
        ...
        try:
            sender_uuid = uuid.UUID(str(sender_id)) if sender_id else None
                ...
                for part in mention_parts[1:]:
                    ...
                    for length in range(min(len(words), 4), 0, -1):
                        ...
                        try:
                            ...
                            user_query = _ctrl.query(User).filter(*base_filter)
                            if entry['side'] == 'SHORE':
                    ...
                    if matched_user and matched_uid != sender_uuid and matched_uid not in processed_uids:
                        ...
                        processed_uids.add(matched_uid)
            ...
            for eq in visible_equipment:
                ...
                if not is_configured:
                    ...
                    if target_priority:
                        ...
                        cooldown_overdue = datetime.utcnow().replace(tzinfo=None) - timedelta(days=15)
                    ...
                    if latest_sample.is_resampling_required and not latest_sample.is_resolved:
                        ...
                        cooldown_resample = datetime.utcnow().replace(tzinfo=None) - timedelta(days=15)
