...
from app.luboil_model import LuboilVessel
# from app.models import VesselInfo
import logging

logger = logging.getLogger(__name__)

def get_or_create_vessel(session: Session, 
    ...
    existing_vessel = session.query(VesselInfo).filter(
        VesselInfo.imo_number == imo_int
    ).first()
    ...
    if existing_vessel:
        ...
        if engine_type_from_pdf and existing_vessel.engine_type != engine_type_from_pdf:
            existing_vessel.engine_type = engine_type_from_pdf
            updated = True
        ...
        new_vessel = VesselInfo(
            ...
            engine_type=engine_type_from_pdf,
        ...
        session.flush()  # Get the auto-generated vessel_id
