...
class AEDataLoader:
    ...
    async def load_all_data(self, extracted_data: Dict[str, List[Dict]]) -> AELoadingStats:
        """Load all AE data."""
        ...
        try:
            async with get_db_session_context() as session:
                ...
                await self._load_generators_batch(session, extracted_data.get('vessel_generators', []))
                # Re-preload generators to ensure new ones are included
                await self._preload_existing_generators(session) 
                await self._load_generator_baseline_batch(session, extracted_data.get('generator_baseline_data', []))
    ...
    async def _preload_existing_generators(self, session: AsyncSession) -> None:
        gens = (await session.execute(select(VesselGenerator))).scalars().all()
    ...
    async def _preload_existing_data(self, session: AsyncSession) -> None:
        ...
        result = await session.execute(
            select(GeneratorBaselineData, VesselGenerator.engine_no)
            .join(VesselGenerator, GeneratorBaselineData.generator_id == VesselGenerator.generator_id)
        )
        for b, engine_no in result.all():
    ...
    async def _load_generators_batch(self, session: AsyncSession, generators_data: List[Dict[str, Any]]) -> None:
    ...
    async def _load_generator_baseline_batch(self, session: AsyncSession, baseline_data: List[Dict[str, Any]]) -> None:
...
async def load_ae_data_to_database(extracted_data: Dict[str, List[Dict]]) -> AELoadingStats:
