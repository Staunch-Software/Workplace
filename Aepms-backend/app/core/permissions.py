...
async def get_allowed_vessel_imos(current_user: dict) -> Tuple[List[int], str]:
    ...
    async with AsyncSessionControl() as control_db:
        try:
            ...
            """)
            result = await control_db.execute(query, {"uid": str(user_id_raw)})
            rows = result.fetchall()
            clean_imos = [int(row[0]) for row in rows if str(row[0]).isdigit()]
            ...
            if role in ("ADMIN", "SUPERUSER", "SHORE", "SUPERINTENDENT"):
                result = await control_db.execute(select(ControlVessel))
